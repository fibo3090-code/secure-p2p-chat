...
- **LAN peer discovery**: `core/src/network/discovery.rs` advertises `_p2p-messenger._tcp.local.` (mDNS/Bonjour) when hosting and browses for peers, yielding `DiscoveredPeer { name, address, port, fingerprint }`. Surfaced by the TUI and the desktop app (`list_discovered_peers` + "Nearby peers" in the connect pane); gated behind `Config::enable_mdns` (off by default — it reveals presence on the LAN). TOFU still applies — discovery only supplies the address, never trust. `merge_resolved` **replaces** a service's previous entries on every resolve (treating same-fullname-different-address as a duplicate stranded a peer that changed IP at its old address, and taking only the first address hid a dual-homed one), records every advertised address up to `MAX_ADDRESSES_PER_PEER`, and caps the list at `MAX_DISCOVERED_PEERS` — with the retain deliberately **before** the cap check, so a flood cannot make a re-announcing real peer vanish from the UI.
...
- **Peer-supplied strings are decoded strictly, never lossily.** `decode_text`
  (`core/src/core/protocol.rs`) refuses invalid UTF-8 rather than repairing it,
  and every legacy ASCII arm bounds its field before decoding. `from_utf8_lossy`
  expands each invalid byte to three, and **every cap in the decoder counts wire
  bytes** — so lossy decoding put a 64 KiB `Text` frame at 192 KiB in memory and
  the 512-chunk ceiling at ~72 MiB against a documented ~24 MiB. It also made
  the decoder accept frames its own encoder cannot reproduce. The round-trip
  property in `core/tests/fuzz_parsers.rs` asserts the **byte** fixpoint, not
  the discriminant: a discriminant comparison passes for a decoder that silently
  alters content, which is the entire bug class it exists for.
- **Bincode frames reject trailing bytes** (`util::decode_exact`). Plain
  `bincode::deserialize` stops at the end of a value and ignores the rest, so
  `frame || junk` decoded to `frame`: two byte strings, one meaning. Applies to
  `IdentityProof`/`Ed25519Proof` (read *before* any trust decision) and to
...
- `core/src/transfer/receiver.rs` — receiving. `IncomingFileSync::new` takes the
  **directory and the filename separately** and sanitises the name itself. It
  used to take one joined path and keep it verbatim, sanitising only the temp
  name, so `finalize` reused the peer's `file_name()` *and* `parent()` raw and a
  name of `../../x` escaped the download folder. Do not collapse those two
  arguments back into a path: the whole traversal defence would rest on the
  decoder again. The declared-size check runs **before** the write, not after:
  checking afterwards still put the excess bytes on disk. `sanitize_filename` (`core/src/util.rs`) budgets **bytes** not chars (`MAX_FILENAME_BYTES`, leaving room for the receiver's `tmp_<uuid>_` prefix under NAME_MAX), defuses Windows device names (`CON`, `NUL`, `COM1`…), strips control characters and bidi overrides (U+202E renders `photo_gnp.exe` as `photo_exe.png`), and trims the trailing dots/spaces Windows would drop silently.
...
- **Fuzzing comes in two halves, and they are not interchangeable.**
  ...
  are tracked, because a fuzz target nobody can find is one nobody runs — and
  Fast Checks runs `cargo check --manifest-path core/fuzz/Cargo.toml`, because a
...
- **Blob I/O happens off the state lock, in two halves.** `handle_request` decides — under the lock, because access control is a question about state — and returns a `Dispatch { deferred: Some(..) }`; `connection.rs` performs the I/O on `spawn_blocking` once the lock is released. Doing it inline meant one member's 100 MiB transfer stalled every other member's messages for its whole duration, and `blocking_io` does **not** fix that: it frees the runtime *worker*, not the *lock*.
  - **Reads**: `plan_blob_read` → `BlobReadPlan::perform`. Authorisation is point-in-time and the window is now wider; content addressing is what makes that safe to state (the path always holds the same bytes), and revocation takes effect at the next request. A blob deleted mid-read answers "unknown file", which by then it is.
  - **Writes** are three-phase — **inline (4 MiB) and chunked (100 MiB) both**, because a rule with an exception is one that gets forgotten: `begin_upload` / `begin_inline_upload` (check, **reserve** the storage, take the bytes) → `write_staging` (hash + fsync, no lock) → `commit_upload` (re-check, rename into place, release the reservation). `post_file`/`post_file_dm` are the convenience form that runs all three back to back for in-process callers and tests; there is no second write path any more (`store_blob`/`write_blob_file` are gone).
  - **The content hash is known before the reservation is taken, on both paths**, so no reservation is made for content the store already holds and a deduplicated re-upload still succeeds at a full ceiling. Inline uploads hash their 4 MiB in hand; chunked uploads accumulate the hash as chunks arrive (`PendingUpload::hasher`), which costs microseconds on work already done under the lock and avoids a 100 MiB pass there. `stage` still handles an unknown hash pessimistically — a duplicate would reserve its size and give it back at commit — so a future caller that genuinely cannot know it is safe rather than wrong, just needlessly strict at a full ceiling. Each phase exists for a reason that bit: the reservation is what makes the ceiling bound *peak disk* rather than only stored bytes; the re-check is because the lock was released for as long as a 100 MiB write takes (`check_may_post_file` is one function called from both ends so they cannot drift — that is how the DM recipient came to be checked only on the way in); and staging under a **per-upload id, never the content hash** is what stops one member's failed commit unlinking bytes another's commit is about to record.
  - Staging files are reclaimed three ways, because each covers a case the others cannot: `StagedUpload::drop` (abandoned in-process), the write-error path (a partial from a full disk), and `sweep_staging_files` at `load` (killed mid-write). `cancel_connection_uploads` releases reservations, which `Drop` cannot reach — it can remove its own file but cannot touch the state counting the bytes.
- **Party file downloads are access-checked before any byte is read**, never through the raw `blob_bytes`. The server's own path is `plan_blob_read` (decide under the lock, read after releasing it); `blob_bytes_for`/`blob_chunk_for` are the in-process and test entry points and *delegate* to it, so there is exactly one place that decides what a read may see. Content-addressed blobs are stored globally (dedup), so the *download endpoint* is what enforces who may see a file. Access is decided over the **`file_refs` table**, not by rescanning messages — a deleted reference has to stop granting access immediately. Blob bytes are read from disk on demand — a disk-backed `PartyState` keeps `BlobRecord.data: None`, so the storage ceiling is not also an RSS ceiling.
...
- **Upload teardown is per *connection*, not per member** (`cancel_connection_uploads`). One person can be signed in from two devices with uploads in flight on both; sweeping by member made closing a laptop cancel the phone's transfer. A chunked upload's reservation is keyed by the client's own upload id, so `ConnState.uploads` is simultaneously the list of spools and the list of reservations to release — which is why an id stays listed *through* the deferred phase and is cleared by `connection.rs` after commit or discard, not by the dispatcher before it. Inline uploads mint an id and `track_upload` records it for the same reason. `cancel_uploads_for` (per member) survives for tests and deliberate resets only.
- **A member's in-flight uploads are counted from `PartyState`, not from the
  connection's own list.** Those two are allowed to drift, and every place that
  trusted the connection's list was a bypass: `FinishUpload` cleared it *before*
  `finish_upload`, which returned `Err` on the incomplete path without dropping
  the spool — so up to `MAX_PARTY_FILE_BYTES` leaked per round, the concurrency
  cap read zero, and the disconnect sweep (gated on the same list) never fired.
  `finish_upload` now drops the spool on every path out of it except a
  stranger's id, and the disconnect sweep runs over the ids the departing
  connection actually had open (see the per-connection teardown entry above).
- **Sequence 0 is the client's sentinel for an unconfirmed send, so an inbound
  envelope carrying it is dropped** (`client/src/app/party_manager.rs`). An
  honest server assigns `len() + 1` and never sends 0; a hostile one that does
  gets its envelope sorted in among the user's pending sends, after which the
  ordered `ActionFailed` retracts the wrong row.
- **Community-server join is rate-limited in two places**: `dispatch::MAX_JOIN_ATTEMPTS` caps rejected joins per connection (so one handshake does not buy unlimited password guesses), and `messenger_core::network::ratelimit::RateLimiter` in the accept loop caps connections per IP (the **relay** server uses the same limiter, plus `RELAY_HELLO_TIMEOUT` and a `MAX_PENDING_RENDEZVOUS` cap — a peer that connects and never speaks used to hold a task and socket forever). Both accept loops log-and-continue on a failed `accept()` rather than `?`, so one transient EMFILE does not take the listener down for the life of the process (so reconnecting is not the guessing loop). The password compare is `subtle`-based and constant-time — `Option<&str>` equality short-circuits on the first differing byte, which leaks the secret a character at a time to anyone who can measure it. `limiter_key` counts IPv6 per /64 but **canonicalises first**: masking an IPv4-mapped address (`::ffff:a.b.c.d`) to a /64 wipes the marker *and* the address, so every IPv4 client keyed to `::` and one of them could spend the limit for all of them. NAT64 (`64:ff9b::/96`) is the same collapse a level up and is unwrapped too.
- **`recv_packet` (`core/src/core/framing.rs`) is DoS-hardened**: rejects oversized length prefixes and reads in 64 KiB chunks. `MAX_PACKET_SIZE` (8 MiB) bounds every frame. Finer per-field caps: message **text** is bounded per frame by `MAX_TEXT_MESSAGE_BYTES` (64 KiB) and per message by 512 × `TEXT_CHUNK_BYTES` ≈ 24 MiB — see protocol notes above; the **Party server** caps usernames (`MAX_USERNAME_CHARS` 32), channel names (`MAX_CHANNEL_NAME_CHARS` 64), and channel/DM text (`MAX_MESSAGE_TEXT_BYTES` = 64 KiB) in `server/src/state.rs`. UI truncation (ellipsis) is display-only, not a wire limit.
