...
- **`blob_chunk_for` seeks and reads one chunk; it must not go through `blob_bytes`.** It used to, so serving 64 KiB of a 100 MiB blob read all 100 MiB off disk — ~160 GiB across a full download's 1,600 chunks, every byte with the state mutex held. Note what `blocking_io` does and does not buy: it frees the runtime *worker*, not the *lock*, so I/O under `state.lock().await` still queues every other connection behind it. Genuinely fixing that means authorising under the lock and moving the bytes outside it; the whole-blob `blob_bytes` path still has the problem.
