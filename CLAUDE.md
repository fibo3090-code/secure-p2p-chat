...
- **Party file downloads must go through `PartyState::blob_bytes_for(member, hash)`** (access-checked), never the raw `blob_bytes`. Content-addressed blobs are stored globally (dedup), so the *download endpoint* is what enforces who may see a file: channels the member may **read** (`member_can_read_channel`), or DM threads they are part of. Blob bytes are read from disk on demand — a disk-backed `PartyState` keeps `BlobRecord.data: None`, so the 1 GiB storage ceiling is not also a 1 GiB RSS ceiling.
...
- **`ChannelKind` is enforced, not decorative.** `Private`/`Locked`/`Announce` were stored, persisted, shipped to clients and checked nowhere. There is still no per-channel membership model, so anything non-`Public` **fails closed**: `member_can_read_channel` denies `Private`, `member_can_post_to_channel` denies all three. `create_channel` only makes `Public` channels, so this changes nothing today — it means the first per-channel ACL cannot ship silently open.
