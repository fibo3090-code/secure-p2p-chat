        ...
        tokio::select! {
            ...
            Some(mut msg) = from_app_rx.recv() => {
                ...
                if let Err(e) = send_packet(&mut stream, &encrypted).await {
                    ...
                    messages_since_rekey += 1;
                    // Report the transport seq of load-bearing final frames back
                    // to the app: FileEnd ("queueing is not delivery"), and the
                    // frame carrying an outgoing text message (single frame, or
                    // the final chunk of a large one) so the app can correlate
                    // the peer's delivery receipt (`Ack { acked_seq }`) back to
                    // that message. Frames drain FIFO per session, so order
                    // correlates.
                    match &msg {
