            ...
            egui::TopBottomPanel::top("top_panel").show(ctx, |ui| {
                egui::menu::bar(ui, |ui| {
                    ...
                    ui.menu_button("⚙️ Settings", |ui| {
                        ...
                        ui.separator();
                        if ui.button("💾 Save History Now").clicked() {
                            if let Ok(manager) = self.chat_manager.try_lock() {
                                if let Err(e) = manager.save_history(&self.history_path) {
                                    tracing::error!("Failed to save history: {}", e);
                                } else {
                                    tracing::info!("History saved successfully");
                                }
                            }
                            ui.close_menu();
                        }
                        if ui.button("🗑️ Clear All History").clicked() {
                            if let Ok(mut manager) = self.chat_manager.try_lock() {
                                manager.chats.clear();
            ...
            if self.show_about {
                egui::Window::new("ℹ️ About")
                    ...
                    .show(ctx, |ui| {
                        ...
                        ui.label("🔐 Forward Secrecy: X25519 ECDH key exchange");
                        ui.label("🔑 Security: Fingerprint verification");
                        ui.label("📁 Features: File transfer, message history");
                        ui.label("💾 Auto-save: Conversation history preserved");
                        ui.add_space(10.0);
