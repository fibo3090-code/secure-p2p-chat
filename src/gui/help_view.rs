...
fn render_about_tab(ui: &mut egui::Ui) {
    ...
    ui.group(|ui| {
        ...
        ui.label(
            "• End-to-End Encryption: Every message is encrypted. Only the recipient can read it.",
        );
        ui.label(
            "• Anonymity: No phone numbers, no email addresses required. Just cryptographic keys.",
        );
    });
}
...
fn render_features_tab(ui: &mut egui::Ui) {
    let features = [
        (
            "🔒 Encryption",
            "RSA-2048-OAEP for key exchange, AES-256-GCM for messages.",
        ),
        (
            "🔐 Forward Secrecy",
            "X25519 ECDH + HKDF for ephemeral session keys.",
        ),
        (
            "📁 File Transfer",
            "Securely shear files of any size (up to configured limit).",
        ),
        (
            "👥 Group Chats",
            "Create private groups with trusted contacts.",
        ),
        (
            "⌨️ Typing Indicators",
            "See when your friends are typing in real-time.",
        ),
        (
            "🎨 Customization",
            "Choose from multiple themes (Light, Dark, Midnight, Forest).",
        ),
    ];
...
fn render_faq_tab(ui: &mut egui::Ui) {
    ...
    let faqs = [
        (
            ...
            "One person must 'Host', and the other must 'Connect' using the host's IP and Port. Or use the 'Invite Link' feature to share connection info easily.",
        ),
        (
            "Is it really secure?",
            "Yes. We use industry-standard cryptographic primitives (RSA, AES-GCM, X25519). Private keys never leave your device.",
        ),
        (
            "Where are files saved?",
            "Files are saved to your configured Download Directory (default: Downloads folder). You can change this in Settings.",
        ),
        (
            "Can I chat over the internet?",
            "Yes, if the Host port is forwarded or if you are using a VPN/Overlay network (like Hamachi, or Tailscale). By default, it works on Local LAN.",
        ),
    ];
