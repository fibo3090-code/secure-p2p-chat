...
fn test_message_roundtrip_integration() {
    ...
    app.input_text = "Integrated message".to_string();
    app.send_message();
...
fn test_multiple_chats_isolation() {
    ...
    // Send to Chat 1
    app.chat_list_state.select(Some(0));
    app.input_text = "To Chat 1".to_string();
    app.send_message();

    // Send to Chat 2
    app.next_chat(); // Switch to Chat 2 (index 1)
    app.input_text = "To Chat 2".to_string();
    app.send_message();
...
fn test_chat_switching_preserves_input_state() {
    ...
    app.input_text = "Draft for chat 0".to_string();

    // Switch to next chat
    app.next_chat();
    assert_eq!(app.chat_list_state.selected(), Some(1));

    // The current TUI design shares app.input_text across all chats!
    // This is useful for accidental switches, or maybe it should be per-chat?
    // Let's verify what the code actually does today.
    // In src/tui/app.rs, input_text is a field of TuiApp, not Chat.
    assert_eq!(app.input_text, "Draft for chat 0");
