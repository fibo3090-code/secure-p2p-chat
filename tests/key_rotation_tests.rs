...
fn test_rekey_sequence_produces_correct_chain() {
    ...
    let mut current_key = [11u8; AES_KEY_SIZE];

    // Perform multiple rekeying operations
    for i in 0..5 {
        let nonce = [i as u8; 16];
        current_key = rekey_session_key(&current_key, &nonce);
        
        // Verify each key is valid and different
        assert_ne!(current_key, [i as u8; AES_KEY_SIZE]);
...
fn test_rekey_timing_performance() {
    ...
    assert!(elapsed.as_millis() < 100, "1000 rekeys took {}ms", elapsed.as_millis());
}
